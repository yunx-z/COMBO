...
for dataset in ["hotpotqa"]:
    for split in ["train", "dev"]:
        ...
        k_dpr = dict()
        with open(f"../atlas/atlas_data/experiments/infer_hotpotqa_large_dpr_10_k_dpr_sample_{split}/{split}-step-0.jsonl", 'r') as reader:
            for line in reader:
                item = json.loads(line)
                if dataset == "wizard" and type(item['EM']) == type(True):
                    continue
                k_dpr[tuple(item['case_idx'])] = ems(item['generation'], item['answers'])
        for item in dataset_items:
            assert (item["idx"], -1, -1) in k_dpr
        k_1_dpr = dict()
        with open(f"../atlas/atlas_data/experiments/infer_hotpotqa_large_dpr_10_k_1_dpr_sample_{split}/{split}-step-0.jsonl", 'r') as reader:
            for line in reader:
                item = json.loads(line)
                if dataset == "wizard" and type(item['EM']) == type(True):
                    continue
                k_1_dpr[tuple(item['case_idx'])] = ems(item['generation'], item['answers'])
        for item in dataset_items:
        ...
        for k_dpr_case_idx in tqdm(k_dpr):
            ...
            for j in range(DPR_CTX_CNT):
                ...
                if (dataset != "wizard" and not k_1_dpr[k_1_dpr_case_idx] and k_dpr[k_dpr_case_idx]) or (dataset == "wizard" and k_dpr[k_dpr_case_idx] - k_1_dpr[k_1_dpr_case_idx] > 0.1):
                    positive.append({"sentence1": f"{question}", "sentence2": f"{dpr_ctx['text']}", "label": "positive", "type": "positive-I", "answer": answer})
                    ...
                    negative.append({"sentence1": f"{question}", "sentence2":  f"{dpr_ctx['text']}", "label": "negative", "type": "negative-I", "answer": answer})
            """
        ...
        """
        positive.sort(key=lambda x:x['sentence1'])
        new_positive = []
        new_negative = []
        for idx, pos in enumerate(tqdm(positive)):
            if positive[max(idx-1,0)]['sentence1'] == pos['sentence1'] or pos['sentence1'] == positive[min(idx+1,len(positive)-1)]['sentence1']:
                continue
            new_positive.append(pos)
            neg = None
            for item in dataset_items:
                if item['question'] == pos['sentence1']:
                    dpr_ctxs_without_pos = [ctx for ctx in item['ctxs'][10:20] if ctx['text'] != pos['sentence2']]
                    assert len(dpr_ctxs_without_pos) == 9
                    neg = {"sentence1": item['question'], "sentence2": random.choice(dpr_ctxs_without_pos)['text'], "label": "negative", "type": "negative-I", "answer": item['answers']}
                    break
            assert neg is not None
            new_negative.append(neg)

        print(f"new_positive: {len(new_positive)} items; new_negative: {len(new_negative)} items")
        ...
        with open(outfile, 'w') as writer:
            for pos in new_positive:
                writer.write(json.dumps(pos)+'\n')
            for neg in new_negative:
