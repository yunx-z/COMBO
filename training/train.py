        ...
        for i, batch in enumerate(data_iterator):
            ...
            if step % opt.eval_freq == 0:
                for data_path in opt.eval_data:
                    ...
                    dev_f1 = metrics['f1']
                    all_dev_em.append((dev_em, dev_f1))
                    if dev_em > best_dev_em[0] and opt.is_main:
                        best_dev_em = (dev_em, dev_f1)
                        save_atlas_model(
                            unwrapped_model,
                            optimizer,
                            scheduler,
            ...
            if step > opt.total_steps:
                if opt.is_main:
                    log = f"best dev EM: {best_dev_em[0]:.3f} F1: {best_dev_em[1]:.3f}\nstep-wise EM: "
                    for dev_em, dev_f1 in all_dev_em:
                        log += f"({dev_em:.3f}, {dev_f1:.3f}) "
                    logger.info(log)
                exit()
